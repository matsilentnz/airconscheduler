            ...
            <tbody>
              <tr v-for="(booking, index) in bookings" :key="index">
                ...
                <td>
                  ...
                  <div class="dropdown d-inline-block" v-if="booking.status !== 'Completed'">
                    <button class="btn btn-sm btn-outline-secondary dropdown-toggle"
                            type="button"
                            :id="'techDropdown-' + booking.id" data-bs-toggle="dropdown"
                            aria-expanded="false">
                      {{ booking.technician_id ? getTechName(booking.technician_id) : 'Assign Tech' }}
                    </button>
                    <ul class="dropdown-menu" :aria-labelledby="'techDropdown-' + booking.id">
                      <li v-for="tech in technicians" :key="tech.id">
                        <a class="dropdown-item"
                           href="#"
                           @click.prevent="assignTechnician(booking, tech.id)">
                          {{ tech.name }}
                        </a>
                      </li>
                      <li v-if="booking.technician_id">
                        <a class="dropdown-item text-danger"
                           href="#"
                           @click.prevent="assignTechnician(booking, null)">
                          Unassign
                        </a>
                      </li>
                    </ul>
              ...
              <div v-if="currentBooking">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    ...
                    <p><strong>Technician:</strong> {{ currentBooking.technician ? currentBooking.technician.name : 'Not assigned' }}</p>
  ...
  methods: {
    ...
    async fetchTechnicians() {
      try {
        ...
        console.log('Technicians data:', this.technicians); // Log the structure
      } catch (error) {
    ...
    async assignTechnician(booking, techId) {
      try {
        console.log('Attempting to assign technician', { bookingId: booking.id, techId });

        const response = await axios.put(
          `http://127.0.0.1:8000/api/admin/bookings/${booking.id}/assign`,
        ...
        this.bookings = this.bookings.map(b =>
          b.id === booking.id ? { ...b, technician_id: techId } : b
        );
